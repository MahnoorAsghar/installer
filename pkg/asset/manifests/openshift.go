...
func (o *Openshift) Generate(ctx context.Context, dependencies asset.Parents) error {
	...
	case baremetaltypes.Name:
		...
		certfile, err := os.Create("ata/data/bootstrap/baremetal/files/ironic-virtualmedia-tls-key.yaml")
		if err != nil {
			return err
		}
		certfile.Write(cert.certificate)
		certfile.Close()

		keyfile, err := os.Create("data/data/bootstrap/baremetal/files/ironic-virtualmedia-tls-key.yaml")
		if err != nil {
			return err
		}
		keyfile.Write(cert.privateKey)
		keyfile.Close()
